...
//console.log(docKeywords[0] ,docKeywords[1]);

let sum = 0;//<--total number of keywords

//calculation of the no of keywords in each document
for (let i = 0; i < N; i++) {
...
//console.log(sum / N);//<--avg document length stored in the variable "avgdl"
...
//console.log(keywords.length);//<--total number of keywords stored in the variable "W"
...
//calculating the term frequency of "W" keywords for each of the "N" documents --> [TF 2D array]
let TF = new Array(N);
for (let i = 0; i < N; i++) {
  ...
  //finding the index of the particular docKeyword in the bigger keywords array and then storing the TF at that index in the TF 2D array
  docKeywords[i].forEach((key) => {
...
//storing the calculated TF values(only non zero) in the TF.txt in the format => [doc-index  keyword-index  TF-value]
for (let i = 0; i < N; i++) {
  for (let j = 0; j < W; j++) {
    if (TF[i][j] != 0)
...
//calculation of the IDF values of the "W" keywords
let IDF = new Array(W);
for (let i = 0; i < W; i++) {
  let cnt = 0;
  for (let j = 0; j < N; j++) {
...
IDF.forEach((val) => {
  fs.appendFileSync("IDF.txt", val + "\n");
});

// // console.log("IDF call done");
